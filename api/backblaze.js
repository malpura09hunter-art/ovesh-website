...
function ownedKey(uid, objectKey) {
  return String(objectKey || '').startsWith(`users/${uid}/`) && !String(objectKey).includes('..');
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'presign-upload') {
      ...
      const objectKey = `users/${user.uid}/${folder}/${crypto.randomUUID()}-${name}`;
    ...
    if (action === 'presign-download') {
      ...
      return res.status(200).json({ ok: true, downloadUrl, expiresIn: 900 });
    }
