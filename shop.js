...
function renderCart(){
 ...
 $('cartItems').innerHTML=cart.length?cart.map((x,i)=>{const id=esc(x.id);const name=esc(x.name);return '<div class="cart-line" data-cart-id="'+id+'" style="--cart-index:'+i+'"><div><strong>'+name+'</strong><button aria-label="Remove '+name+'" onclick="removeCart(\\''+id+'\\')">×</button></div><div class="qty"><span>'+money(x.price*x.qty)+'</span><span><button aria-label="Decrease quantity" onclick="changeQty(\\''+id+'\\',-1)">−</button> '+x.qty+' <button aria-label="Increase quantity" onclick="changeQty(\\''+id+'\\',1)">+</button></span></div></div>'}).join(''):'<p class="muted">Your cart is empty.<br><small>Add a service to begin your request.</small></p>';
