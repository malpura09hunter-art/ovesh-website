...
function renderCart(){$('cartCount').textContent=cart.reduce((a,x)=>a+x.qty,0);$('cartItems').innerHTML=cart.length?cart.map(x=>'<div class="cart-line"><div><strong>'+esc(x.name)+'</strong><button aria-label="Remove '+esc(x.name)+'" onclick="removeCart(\''+esc(x.id)+'\')">×</button></div><div class="qty"><span>'+money(x.price*x.qty)+'</span><span><button aria-label="Decrease quantity" onclick="changeQty(\''+esc(x.id)+'\',-1)">−</button> '+x.qty+' <button aria-label="Increase quantity" onclick="changeQty(\''+esc(x.id)+'\',1)">+</button></span></div></div>').join(''):'<p class="muted">Your cart is empty.<br><small>Add a service to begin your request.</small></p>';$('cartTotal').textContent=money(cart.reduce((a,x)=>a+x.price*x.qty,0));$('checkoutBtn').disabled=!cart.length}
